...
Command: npx gltfjsx@6.2.18 public/models/globe.glb -o src/components/Heart.jsx -r public 
*/
...
export function Model({clicked, ...props}) {
  const group = useRef();
  ...
  useFrame((state, delta) => {
    if (clicked) {
      setProgress((prev) => Math.min(prev + delta * 1.5, 1)); // Smooth transition speed
    } else {
      setProgress((prev) => Math.max(prev - delta * 1.5, 0)); // Smooth transition speed
    }

    group.current.children.forEach((mesh) => {
      if (progress > 0) {
        if (mesh.name === 'Sketchfab_model') {
          mesh.visible = false;
        } else {
          mesh.visible = true;
        }
        ...
        mesh.rotation.x = THREE.MathUtils.lerp(
          mesh.originalRotation.x*10,
          mesh.targetRotation.x*10,
          10-progress
        );
        mesh.rotation.y = THREE.MathUtils.lerp(
          mesh.originalRotation.y*10,
          mesh.targetRotation.y*10,
          10-progress
        );
        mesh.rotation.z = THREE.MathUtils.lerp(
          mesh.originalRotation.z*10,
          mesh.targetRotation.z*10,
          10-progress
        )
      }
    });
